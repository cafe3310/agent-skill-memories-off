...
- **[▶️执行方式]**: Agent 先 `read_file` 确认内容，再用 `replace` 或脚本精确追加。
- **输入参数**:
  - `libraryName`: 目标知识库名称。
  - `entityName`: 目标实体名称。
  - `inSection`: 目标章节标题（不含 `#`）。
...
### 1.2 `replaceSection`
- **职能**: 完全替换现有章节（包括标题和正文）。
- **[▶️执行方式]**: Agent 先 `read_file` 确认位置，再使用 `replace` 精确修改。
- **输入参数**:
