...
- **[▶️执行方式]**: `python scripts/stats.py --path <path>` (无需 commit)
- **输入参数**:
  - `libraryName`: 目标知识库名称。
  - `reason` (可选): 执行操作的原因。
- **输出报告 (XML)**:
...
- **[▶️执行方式]**: Agent 直接使用 `read_file` 读取根目录下的 `meta.md`。(无需 commit)
- **输入参数**:
  - `libraryName`: 目标知识库名称。
- **输出报告 (XML)**:
...
- **[▶️执行方式]**: `python scripts/inspect_schema.py --path <path> --mode types` (无需 commit)
- **输入参数**:
  - `libraryName`: 目标知识库名称。
- **输出报告 (XML)**:
  - `entity_types`:
...
- **[▶️执行方式]**: `python scripts/inspect_schema.py --path <path> --mode keys` (无需 commit)
  - `libraryName`: 目标知识库名称。
- **输出报告 (XML)**:
  - `metadata_keys`:
